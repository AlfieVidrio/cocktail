import {
    html,
    render
  } from "lit-html";
  
  let searchBar = document.getElementById("search")
  searchBar.addEventListener("keyup", function(event) {
    if (event.key === "Enter") {
      // Execute search function with search term
      doSearch();
    }
  });
  function doSearch() {
    let searchStr = searchBar.value;
    let baseURL = "https://www.thecocktaildb.com/api/json/v1/1/search.php?s="
    let newURL = baseURL.concat(searchStr)
  
    // Fetch data from jsonplaceholder's "users" endpoint
    fetch(newURL)
      // Then convert the response to JSON
      .then((response) => response.json())
  
      // Then do something with the JSON data
      .then((data) => {
        let drinks = data;
  
        // Use .map to create an array of html templates to render to the DOM
        let drinkList = drinks.drinks.map(function(drink) {
          return html`<div @click=${()=>renderRecipe(drink)} class="user-entry">
          <h2 id="${drink.strDrink}">${drink.strDrink}</h2>
        </div>`;
        });
  
        // Render the userList array to the user-list div
        render(drinkList, document.getElementById("user-list"));
      });
  }
  
  function renderRecipe(drink) {
    // items is the dictionary that contains the arrays that contain the drink info
    let items = Object.entries(drink);
    let drinkName; 
  
    console.log(drinkFormatter(items))
    
    // renders the content
    let itemTemplates = items.map(function(item) {
      if (item[0] == "strDrink") {
        drinkName = item[1]
      }
  
      let newRecipe = drinkFormatter(items)
  
      if (item[1] !== null) {
        return html`
        <div class="new">
        <p>${item[1]}</p>
      </div>`;}
    });
    render(itemTemplates, document.getElementById(drinkName));
  }
  
  function drinkFormatter(drink_dict) {
    // console.log(drink_dict)
    const newDict = {};
    newDict["Instructions"] = drink_dict[9][1].split('. ');
    
    for (const [key, value] of Object.entries(drink_dict)) {
      if (key.startsWith('strIngredient') && value !== null) {
        console.log(`strMeasure${key.slice(13)}`)
      }
    }
  
  
  }